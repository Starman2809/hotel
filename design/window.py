...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QLabel, QGridLayout, QSpacerItem, QSizePolicy)

from design.button import SimpleButton


class QTAllActionsWindow(QWidget):
    window_title = "Панель управления"
    ...
    def add_objects_to_form(self, main_layout):
        ...
        for index, (field_name, field_callback_class) in enumerate(self.items_to_draw.items()):
            label = QLabel(f"{field_name}:")
            label.setStyleSheet("color: #333; font-size: 16px;")

            create_button = SimpleButton("Создать", field_callback_class["create"].show)
            read_button = SimpleButton("Просмотр, Редактирование, Удаление", field_callback_class["read"])
